...
function Configure-NAT {
    ...
    if ($NetNatAvailable) {
        ...
        try {
            New-NetNat @natDefinition -Confirm:$false
            Add-NetNatExternalAddress -NatName $natDefinition.Name -IPAddress $externalConnection.Address -PortStart 44000 -PortEnd 48000
        } catch {
